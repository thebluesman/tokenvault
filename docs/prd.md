...
**Phases 1–3, 8, 9 are locked and landed** — see below. **Phases 10–13 were scoped 2026-09-04**, in a discussion with Shyam grounded in `docs/ux/user-journeys.md` §13–14 (the Phase 10 candidate-gaps survey): Phase 10 (UI polish, token routing rules, pipeline visibility), Phase 11 (authoring completeness), Phase 12 (Style Dictionary work), Phase 13 (Figma publishing, folding in the publish-target decision deferred from Phase 9) — all provisional until each is individually ticketed.
...
10. **UI polish, token routing rules, pipeline visibility** — provisional, scoped 2026-09-04; ticketed as issues #21–#26. Dark mode for the panel's own chrome ✅ **landed 2026-09-04** via PR #27 (issue #21), built against `docs/ux/dark-mode.md`; all three items the spec left for live verification (contrast, `--bg-subtle`, font smoothing) checked by Shyam in Figma desktop and confirmed fine as shipped. That same live-testing pass also surfaced and fixed an unplanned bug: reference-valued colour token swatches rendered at 55% opacity with a dashed outline, making light/dark-extreme references look inaccurate against Figma's own Variables panel — fixed via issue #28/PR #29, `docs/ux/local-editor.md` §4.5 amended (a reference swatch now renders identically to a literal). Onboarding/first-run polish (issue #22) (`@ux-designer` to scope: PAT setup friction, a bulk subtype-confirmation affordance, an empty-state explaining the three-place model, first-scan chip wording). A fix for subtype confirmations not surviving a push/pull to a second machine — a Phase 6 wiring gap (`extractUserSubtypes` in `src/tokens/subtype.ts` was written but never called from the sync path). Configurable token naming rules (strip/transform path segments by pattern match, re-evaluated live against Figma's current names rather than baked in per token) plus arbitrary multi-repo push routing (any number of configured repos, default-all with pattern-based exceptions) — `@tech-lead` designs an ADR-0002 amendment and an ADR-0006 extension first; no build starts until that lands. Sub-key references on composite tokens (e.g. point just a typography token's font size at another token). Pipeline visibility in the panel — export build status, a warning when `tokensDir` disagrees between Settings and the GitHub Actions workflow, and surfacing when CI failed specifically because of a cycle — all GitHub-API-only, no Style Dictionary changes.
11. **Authoring completeness** — provisional, scoped 2026-09-04. Everything deferred out of Phase 10's authoring-gap discussion: token creation (the consumer-side blocker that justified deferring it in Phase 4 has expired), manual path rename (needs a rename operation added to ADR-0004 first, then reference-rewriting), theme composition (actually building/editing themes in the plugin, not just picking one — ADR-0007 §7b already designs it, unbuilt), `bind to selected layers` (fully spec'd in `apply-and-drift.md` §5.4, zero code), and materializing a pulled-from-repo token as a real Figma Variable (ADR-0006 §11).
12. **Style Dictionary work** — provisional, scoped 2026-09-04. Any future export-pipeline work that touches Style Dictionary's own build/config, kept separate from Phase 10's pipeline-visibility work (which is GitHub-API-only and touches no Style Dictionary internals).
13. **Figma publishing** — provisional, renumbered 2026-09-04 from its original Phase 11 slot. Decide and execute the publish target (private install vs. Figma Community listing) — the decision explicitly deferred out of Phase 9 (§9 above, PRD §6.7's "decide whether to publish privately or to Figma Community" line moved here from its original home in the old Phase 9 scope).
